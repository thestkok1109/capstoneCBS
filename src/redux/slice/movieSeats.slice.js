...
const movieSeats = createSlice({
  name: "movieSeats",
  initialState,
  reducers: {
    datGhe: (state, action) => {
      const { soGhe, daDat } = action.payload;
      console.log("soGhe", soGhe);
      state.danhSachGhe.find((row) => {
        return row.danhSachGhe.find((ghe) => {
          if (ghe.soGhe === soGhe) {
            ghe.daDat = daDat;
            return true; // Stop searching once we find the seat
          }
...
export const { datGhe } = movieSeats.actions;

export default movieSeats.reducer;
